...
$lista_imprensa = getTodasImprensa("DESC");
...
<body>
    ...
    <div class="quadro">
        ...
        <table>
            ...
            <?php foreach($lista_imprensa as $i): ?>

                <tr>
                    <td>
                        <?php if($i["imagem"] != ""): ?>
                            <img src="<?= $i["imagem"]; ?>" alt="Capa Livro">
                        <?php else: ?>
                            SEM IMAGEM
                        <?php endif; ?>
                    </td>
                    <td><?= $i["titulo"]; ?></td>
                    <td>
                        <?php if(strlen($i["texto"]) > 0 && strlen($i["texto"]) < 100): ?>
                            <?= $i["texto"]; ?>
                        <?php else: ?>
                            <?= substr($i["texto"], 0, 100) . "..."; ?>
                        <?php endif; ?>
                    </td>
                    <td><?= $i["data_publicacao"]; ?></td>
                    <td>
                        <a href="../backoffice/actions/imprensa-center.php?editar=<?= $i["id"]; ?>">Editar</a>
                        <br>
                        <a href="../backoffice/actions/imprensa-center.php?deletar=<?= $i["id"]; ?>">Deletar</a>
                    </td>
                </tr>
        ...
        <a href="../backoffice/actions/imprensa-center.php">
            <button class="botoes">Criar Nova POSTAGEM</button>
        </a>
    </div>
